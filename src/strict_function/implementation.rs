...
impl StrictFunction {
    ...
    pub fn call_complex(
        ...
        args: JsValue,
        context: JsValue,
    ) -> Result<StrictFunctionResult, JsValue> {
    ...
    fn convert_to_numeric(
        ...
        if let Some(num) = arg_val.as_f64() {
            ...
            s.parse::<f64>()
                ...
                .map_err(|_| {
                    self.enhance_error_with_type_info(
    ...
    fn validate_any_argument(&self, index: usize, arg_val: JsValue) -> Result<JsValue, JsValue> {
        ...
        if arg_val.is_symbol() && !self.supports_operation("symbol") {
            return Err(JsValue::from_str(&format!(
                "Extra argument {}: symbol not supported by function capabilities",
                index
            )));
        }
        Ok(arg_val)
    ...
    fn enhance_error_with_type_info(
        &self,
        error: JsValue,
        index: usize,
        _operation: &str,
 ...
 fn validate_and_wrap_result(&self, result: JsValue) -> Result<StrictFunctionResult, JsValue> {
    ...
    fn validate_numeric_argument(&self, arg_val: JsValue, index: usize) -> Result<(), JsValue> {
        if !arg_val.as_f64().is_some() && !arg_val.is_string() && !arg_val.as_bool().is_some() {
            return Err(JsValue::from_str(&format!(
                "Argument {} must be a number, string, or boolean",
                index
            )));
        }
        ...
        if arg_val.is_string() {
            ...
            if str_val.parse::<f64>().is_err() {
                return Err(JsValue::from_str(&format!(
                    "Argument {}: string cannot be converted to number",
                    index
                )));
            }
        }
    ...
    pub fn call(&self, args: JsValue) -> Result<f64, JsValue> {
        ...
        let result = self
            .js_function
            .apply(&JsValue::NULL, &processed_args)
   ...
   fn process_result(&self, result: JsValue) -> Result<f64, JsValue> {
    ...
    pub fn supports_operation(&self, operation: &str) -> bool {
        ...
        let args_support = self
            .arg_types
            .iter()
            .all(|arg_type| arg_type.supports_operation(operation));
    ...
    pub fn get_type_compatibility(&self, args: JsValue) -> Result<JsValue, JsValue> {
        ...
        for i in 0..args_array.length().min(self.arg_types.len() as u32) {
            ...
            if compatible {
                ...
                Reflect::set(
                    &info,
                    &"recommendedOperations".into(),
                    &recommended_array.into(),
                )?;
            }
    ...
    pub fn chain(&self, next_function: &StrictFunction) -> Result<StrictFunction, JsValue> {
        if next_function.arg_types.is_empty() {
            return Err(JsValue::from_str(
                "Next function must accept at least one argument",
            ));
        }
        ...
        Reflect::set(
            &chained_js_func,
            &"secondFunc".into(),
            &next_function.js_function,
        )?;
        Reflect::set(
            &chained_js_func,
            &"argCount".into(),
            &(self.arg_types.len() as u32).into(),
        )?;

        StrictFunction::new(
            chained_js_func,
            combined_arg_array.into(),
            next_function.return_type,
        )
    }
